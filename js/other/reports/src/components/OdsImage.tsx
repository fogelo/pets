...
type Props = {};
export const OdsImage = (props: Props) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (file && image) {
            ...
            for (let i = 0; i < cells.length; i++) {
                ...
                if (textNode && textNode.textContent.includes('{{image_placeholder}}')) {
                    textNode.textContent = ''; // Очищаем текстовое содержимое
            ...
            const newZipContent = await loadedZip.generateAsync({ type: 'blob', mimeType: 'application/vnd.oasis.opendocument.spreadsheet' });
