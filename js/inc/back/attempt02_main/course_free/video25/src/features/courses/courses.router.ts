...
coursesRouter
  .get(
    ...
    async (
      ...
      const dbCourses = await coursesRepository.findCourses(title);
  ...
  .post(
    ...
    async (req: RequestWithBody<CreateCourseModel>, res) => {
      ...
      const dbCourse = await coursesRepository.createCourse(newCourse);
  ...
  .put(
    ...
    async (
      ...
      const isCourseUpdated = await coursesRepository.updateCourse(id, title);
      if (!isCourseUpdated) {
        res.sendStatus(HTTP_STATUSES.NOT_FOUND_404);
        return;
      }
  ...
  .delete("/:id", async (req: RequestWithParams<URIParamsCourseModel>, res) => {
    const id = +req.params.id;
    const isCourseDeleted = await coursesRepository.deleteCourse(id);
    if (!isCourseDeleted) {
      res.sendStatus(HTTP_STATUSES.NOT_FOUND_404);
      return;
    } else {
